...
pub async fn start_extension_host() -> Result<(), String> {
    info!("start_extension_host called - extension host is managed by SessionManager");
    Ok(())
}
...
pub fn get_extensions_dir(app_dirs: State<'_, AppDirectories>) -> Result<String, String> {
    app_dirs
        ...
        .to_str()
        .ok_or_else(|| {
            ExtensionHostError::ExtensionNotFound(
                "Failed to convert extensions directory path to UTF-8 string".to_string(),
            )
            .to_string()
        })
        .map(String::from)
...
pub async fn install_extension(
    vsix_path: String, session: State<'_, Arc<RwLock<SessionManager>>>,
...
pub async fn uninstall_extension(
    session: State<'_, Arc<RwLock<SessionManager>>>, extension_id: String,
) -> Result<(), String> {
    let session = session.read().await;
...
pub async fn search_marketplace(
    query: String, page: Option<i32>, page_size: Option<i32>,
...
pub async fn install_from_marketplace(
    session: State<'_, Arc<RwLock<SessionManager>>>, publisher: String, name: String,
    version: String,
) -> Result<InstalledExtension, String> {
    let session = session.read().await;
    session.install_marketplace_extension(publisher, name, version).await
}

#[tauri::command]
pub async fn extension_tree_get_children(
    session: State<'_, Arc<RwLock<SessionManager>>>, view_id: String, element: Option<Value>,
...
pub async fn extension_tree_get_item(
    session: State<'_, Arc<RwLock<SessionManager>>>, view_id: String, element: Value,
) -> Result<Value, String> {
    let session = session.read().await;
...
pub async fn extension_execute_command(
    session: State<'_, Arc<RwLock<SessionManager>>>, command: String, args: Vec<Value>,
) -> Result<Value, String> {
    let session = session.read().await;
...
pub async fn extension_tree_notify_event(
    session: State<'_, Arc<RwLock<SessionManager>>>, view_id: String, event: String,
    element: Option<Value>, selection: Option<Vec<Value>>, visible: Option<bool>,
) -> Result<(), String> {
    let session = session.read().await;
    session.notify_extension_tree_event(view_id, event, element, selection, visible).await
}
