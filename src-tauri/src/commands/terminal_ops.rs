use crate::terminal::{TerminalInfo, TerminalManager, TerminalOptions, TerminalProfile};
use std::sync::Mutex;
use tauri::{AppHandle, State};

#[tauri::command]
pub fn create_terminal(
    name: Option<String>, shell: Option<String>, cwd: Option<String>, app_handle: AppHandle,
    terminal_manager: State<Mutex<TerminalManager>>,
) -> Result<String, String> {
    let manager = terminal_manager.lock().expect("terminal_manager lock poisoned");
...
pub fn create_terminal_with_options(
    options: TerminalOptions, app_handle: AppHandle,
    terminal_manager: State<Mutex<TerminalManager>>,
) -> Result<String, String> {
    let manager = terminal_manager.lock().expect("terminal_manager lock poisoned");
