...
pub async fn parse_and_register_extension_menus(
    extension_id: String, package_json: Value, menu_registry: State<'_, MenuRegistry>,
) -> Result<usize, String> {
    let contributes =
    ...
    let menus_obj =
        menus.ok_or("Menus value is null")?.as_object().ok_or("Menus must be an object")?;
    ...
    for (location, items) in menus_obj {
        let items_array =
            items.as_array().ok_or(format!("Menu items for {} must be an array", location))?;

        for item_value in items_array {
            let item_obj = item_value.as_object().ok_or("Menu item must be an object")?;
            ...
            let when = item_obj.get("when").and_then(|v| v.as_str()).map(|s| s.to_string());

            let group = item_obj.get("group").and_then(|v| v.as_str()).map(|s| s.to_string());

            let title = item_obj.get("title").and_then(|v| v.as_str()).map(|s| s.to_string());

            let icon = item_obj.get("icon").and_then(|v| {
            ...
            let alt = item_obj.get("alt").and_then(|v| v.as_str()).map(|s| s.to_string());
...
pub async fn parse_command_contributions(
    ...
    let commands_array =
        commands.ok_or("Commands value is null")?.as_array().ok_or("Commands must be an array")?;

    let mut contributions = Vec::new();

    for cmd_value in commands_array {
        let cmd_obj = cmd_value.as_object().ok_or("Command must be an object")?;

        let command_id = cmd_obj
            .get("command")
            .and_then(|v| v.as_str())
        ...
        let category = cmd_obj.get("category").and_then(|v| v.as_str()).map(|s| s.to_string());

        let icon = cmd_obj.get("icon").and_then(|v| {
            if let Some(s) = v.as_str() {
                Some(s.to_string())
            } else if let Some(obj) = v.as_object() {
                obj.get("dark")
        ...
        contributions.push(CommandContribution { command: command_id, title, category, icon });
    }
