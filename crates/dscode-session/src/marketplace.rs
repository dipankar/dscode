...
pub(crate) struct MarketplaceExtension {
    pub(crate) publisher: String,
    pub(crate) name: String,
    pub(crate) display_name: String,
    pub(crate) version: String,
    pub(crate) description: Option<String>,
    pub(crate) install_count: i64,
...
pub(crate) async fn download_extension(
    ...
    info!(url = %download_url, "Downloading extension from marketplace");
    ...
    if bytes.len() >= 4 && (&bytes[0..2] != b"PK") {
        ...
        error!(
            first_bytes = ?&bytes[0..std::cmp::min(16, bytes.len())],
            "Downloaded file is not a valid ZIP archive"
        );
        error!(preview = %preview, "File content preview");
        return Err("Downloaded file is not a valid ZIP archive (invalid magic number). \
             This usually means the marketplace API returned an error page instead of the extension file.".to_string());
    }
